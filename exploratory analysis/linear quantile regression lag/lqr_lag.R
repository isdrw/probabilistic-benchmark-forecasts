...
with_progress({
  ...
  weo_list <- future_pmap(
    ...
    function(country, tau, target, horizon) {
      suppressWarnings({
        res <- fit_lqr_lag(df_weo, country, tau, target, horizon)    
      })
...
with_progress({
  ...
  rw_list <- future_pmap(
    ...
    function(country, tau, target, horizon) {
      suppressWarnings({
        res <- fit_lqr_lag(df_rw, country, tau, target, horizon)  
      })
...
with_progress({
  ...
  ar1_list <- future_pmap(
    ...
    function(country, tau, target, horizon) {
      suppressWarnings({
        res <- fit_lqr_lag(df_ar1, country, tau, target, horizon)
      })
...
with_progress({
  ...
  arima1_1_0_list <- future_pmap(
    ...
    function(country, tau, target, horizon) {
      suppressWarnings({
        res <- fit_lqr_lag(df_arima1_1_0, country, tau, target, horizon)
      })
...
with_progress({
  ...
  arima_auto_list <- future_pmap(
    ...
    function(country, tau, target, horizon) {
      suppressWarnings({
        res <- fit_lqr_lag(df_arima_auto, country, tau, target, horizon)
      })
...
with_progress({
  ...
  rw_q_list <- future_pmap(
    ...
    function(country, tau, target, horizon) {
      suppressWarnings({
        res <- fit_lqr_lag(df_rw_q, country, tau, target, horizon, R = 44)
      })
...
with_progress({
  ...
  ar1_q_list <- future_pmap(
    ...
    function(country, tau, target, horizon) {
      suppressWarnings({
        res <- fit_lqr_lag(df_ar1_q, country, tau, target, horizon, R = 44)
      })
...
with_progress({
  ...
  arima1_1_0_q_list <- future_pmap(
    ...
    function(country, tau, target, horizon) {
      suppressWarnings({
        res <- fit_lqr_lag(df_arima1_1_0_q, country, tau, target, horizon, R = 44)
      })
...
with_progress({
  ...
  arima_auto_q_list <- future_pmap(
    ...
    function(country, tau, target, horizon) {
      suppressWarnings({
        res <- fit_lqr_lag(df_arima_auto_q, country, tau, target, horizon, R = 44)
      })
...
write.csv(pred_weo, paste0(
  "results/linear_quantile_regression_lag/ldr_lag_prediction_weo_", 
  timestamp, ".csv"), row.names = FALSE)

write.csv(pred_weo_eval, paste0(
  "results/linear_quantile_regression_lag/ldr_lag_prediction_weo_eval_", 
  timestamp, ".csv"), row.names = FALSE)
...
write.csv(pred_rw, paste0(
  "results/linear_quantile_regression_lag/ldr_lag_prediction_rw_", 
  timestamp, ".csv"), row.names = FALSE)

write.csv(pred_rw_eval, paste0(
  "results/linear_quantile_regression_lag/ldr_lag_prediction_rw_eval_", 
  timestamp, ".csv"), row.names = FALSE)
...
write.csv(pred_ar1, paste0(
  "results/linear_quantile_regression_lag/ldr_lag_prediction_ar1_", 
  timestamp, ".csv"), row.names = FALSE)

write.csv(pred_ar1_eval, paste0(
  "results/linear_quantile_regression_lag/ldr_lag_prediction_ar1_eval_", 
  timestamp, ".csv"), row.names = FALSE)
...
write.csv(pred_arima1_1_0, paste0(
  "results/linear_quantile_regression_lag/ldr_lag_prediction_arima1_1_0_", 
  timestamp, ".csv"), row.names = FALSE)

write.csv(pred_arima1_1_0_eval, paste0(
  "results/linear_quantile_regression_lag/ldr_lag_prediction_arima1_1_0_eval_", 
  timestamp, ".csv"), row.names = FALSE)

#==============================================================================
##Evaluation of prediction on dataset ARIMA auto (quarterly, generated)
...
write.csv(pred_arima_auto, paste0(
  "results/linear_quantile_regression_lag/ldr_lag_prediction_arima_auto_", 
  timestamp, ".csv"), row.names = FALSE)

write.csv(pred_arima_auto_eval, paste0(
  "results/linear_quantile_regression_lag/ldr_lag_prediction_arima_auto_eval_", 
  timestamp, ".csv"), row.names = FALSE)
...
#wont have predictions for h=0.0 because lagged value is identical with truth value
...
write.csv(pred_rw_q, paste0(
  "results/linear_quantile_regression_lag/quarterly/ldr_lag_prediction_rw_q_", 
  timestamp, ".csv"), row.names = FALSE)

write.csv(pred_rw_q_eval, paste0(
  "results/linear_quantile_regression_lag/quarterly/ldr_lag_prediction_rw_q_eval_", 
  timestamp, ".csv"), row.names = FALSE)
...
write.csv(pred_ar1_q, paste0(
  "results/linear_quantile_regression_lag/quarterly/ldr_lag_prediction_ar1_q_", 
  timestamp, ".csv"), row.names = FALSE)

write.csv(pred_ar1_q_eval, paste0(
  "results/linear_quantile_regression_lag/quarterly/ldr_lag_prediction_ar1_q_eval_", 
  timestamp, ".csv"), row.names = FALSE)
...
write.csv(pred_arima1_1_0_q, paste0(
  "results/linear_quantile_regression_lag/quarterly/ldr_lag_prediction_arima1_1_0_q_", 
  timestamp, ".csv"), row.names = FALSE)

write.csv(pred_arima1_1_0_q_eval, paste0(
  "results/linear_quantile_regression_lag/quarterly/ldr_lag_prediction_arima1_1_0_q_eval_", 
  timestamp, ".csv"), row.names = FALSE)
...
write.csv(pred_arima_auto_q, paste0(
  "results/linear_quantile_regression_lag/quarterly/ldr_lag_prediction_arima_auto_q_", 
  timestamp, ".csv"), row.names = FALSE)

write.csv(pred_arima_auto_q_eval, paste0(
  "results/linear_quantile_regression_lag/quarterly/ldr_lag_prediction_arima_auto_q_eval_", 
  timestamp, ".csv"), row.names = FALSE)
